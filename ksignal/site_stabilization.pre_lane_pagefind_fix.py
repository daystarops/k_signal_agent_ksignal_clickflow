...
_L=importlib.machinery.SourceFileLoader("ksignal._stabilization_base",str(_PATH))
_S=importlib.util.spec_from_loader(_L.name,_L); _M=importlib.util.module_from_spec(_S); _L.exec_module(_M)
# Copy only the public alias/index helpers; private loader state must not replace ours.
